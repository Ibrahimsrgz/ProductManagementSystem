...
using ProductManagementSystem.Maui.Storage;
...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(ProductManagementSystemConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(ProductManagementSystemConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(ProductManagementSystemConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(ProductManagementSystemConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(ProductManagementSystemConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(ProductManagementSystemConsts.OidcConsts.RefreshTokenKeyName);
    }
}